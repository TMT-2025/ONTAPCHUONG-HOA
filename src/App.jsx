...
function App() {
  ...
  return (
    <div class="relative min-h-screen bg-gradient-to-tr from-slate-50 via-slate-100 to-teal-50 bg-radial-glow bg-radial-blue-glow bg-radial-purple-glow pb-20 text-slate-800">
      
      {/* HEADER SECTION */}
      <header class="max-w-6xl mx-auto pt-8 px-6 flex items-center justify-between">
        <div class="flex items-center gap-3">
          <div class="h-12 w-12 rounded-xl bg-gradient-to-tr from-teal-500 to-emerald-400 flex items-center justify-center shadow-lg shadow-teal-500/10 border border-teal-400/20">
            <Beaker class="h-6 w-6 text-white stroke-[2.5]" />
          </div>
          <div>
            <h1 class="font-display font-extrabold text-2xl tracking-tight bg-gradient-to-r from-slate-900 via-teal-950 to-teal-900 bg-clip-text text-transparent">
              Chemistry Review Generator
            </h1>
            <p class="text-xs text-slate-600 font-medium">Bộ tạo tài liệu ôn tập chương Hóa học THPT • AI Powered</p>
          </div>
        </div>

        <button 
          onClick={() => setShowSettings(!showSettings)}
          class="flex items-center gap-2 px-3.5 py-2 rounded-lg bg-white border border-slate-200 text-slate-700 hover:text-teal-600 hover:border-teal-500/30 transition-all font-semibold text-sm glass-panel"
        >
      ...
      <main class="max-w-6xl mx-auto mt-8 px-6 grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div class="lg:col-span-5 flex flex-col gap-6">
          ...
          {showSettings && (
            <div class="rounded-xl glass-panel p-5 border border-teal-500/10 shadow-xl relative overflow-hidden">
              ...
              <h2 class="font-display font-bold text-lg text-teal-700 mb-4 flex items-center gap-2">
              ...
              <div class="flex flex-col gap-4">
                <div>
                  ...
                  <input 
                    ...
                    placeholder="AIzaSy..."
                    class="w-full px-3 py-2 bg-white border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:border-teal-500 font-mono text-sm shadow-sm"
                  />
                  <p class="text-[10px] text-slate-600 mt-1 flex items-start gap-1">
                    <Info class="h-3.5 w-3.5 text-teal-600 shrink-0 mt-0.5" />
                ...
                <div class="flex gap-2.5 mt-2 justify-end">
                  <button 
                    ...
                    class="px-4 py-2 text-xs font-semibold text-slate-500 hover:text-slate-800"
                  >
                    Hủy
                  </button>
                  <button 
                    onClick={saveSettings}
                    class="px-4 py-2 bg-teal-600 hover:bg-teal-500 text-white font-bold text-xs rounded-lg shadow-md shadow-teal-600/10 transition-all"
                  >
                    Lưu cấu hình
                  </button>
                </div>
          ...
          {/* SELECTOR PANEL */}
          <div class="rounded-xl glass-panel p-6 border border-slate-200/80 shadow-lg">
            <h2 class="font-display font-bold text-lg text-slate-800 mb-5 flex items-center gap-2">
            ...
            <div class="flex flex-col gap-4">
              {/* Grade Selector */}
              <div>
                <label class="block text-xs font-extrabold text-slate-600 uppercase tracking-wider mb-2">Chọn lớp học</label>
                  ...
                  {['10', '11', '12'].map(g => (
                    <button
                      ...
                      class={`py-2.5 px-3 rounded-lg border font-bold text-sm tracking-wide transition-all ${
                        grade === g 
                          ? 'bg-gradient-to-tr from-teal-50 to-teal-100/50 border-teal-500 text-teal-800 shadow-sm shadow-teal-500/5' 
              ...
              <div>
                <label class="block text-xs font-extrabold text-slate-600 uppercase tracking-wider mb-2">Loại chương trình</label>
              ...
              <div>
                ...
                <select
                  ...
                  class="w-full px-3 py-2.5 bg-white border border-slate-200 rounded-lg text-slate-800 focus:outline-none focus:border-teal-500 font-semibold text-sm shadow-sm"
                >
              ...
              {activeChapterData && (
                <div class="mt-2 p-3.5 rounded-lg bg-slate-100 border border-slate-200">
                  <span class="block text-[10px] font-extrabold text-teal-700 uppercase tracking-wider mb-1">
                  ...
                  <ul class="mt-2 flex flex-col gap-1.5">
                    {activeChapterData.lessons.map((lesson, idx) => (
                      <li key={idx} class="text-[11px] text-slate-600 flex items-start gap-1.5 font-semibold leading-relaxed">
              ...
              <button
                ...
                class="w-full mt-4 py-3 bg-gradient-to-r from-teal-600 to-emerald-500 hover:from-teal-500 hover:to-emerald-400 disabled:from-slate-200 disabled:to-slate-200 text-white disabled:text-slate-400 font-extrabold text-sm rounded-xl tracking-wider uppercase transition-all shadow-md shadow-teal-600/10 hover:shadow-teal-500/20 flex items-center justify-center gap-2 border border-teal-500/20 disabled:border-none"
              >
                {isGenerating ? (
                  <>
          ...
          {/* INSTRUCTIONS NOTICE */}
          <div class="rounded-xl glass-panel p-5 border border-slate-200 text-xs text-slate-600 leading-relaxed shadow-sm">
            <span class="block font-extrabold text-teal-700 uppercase tracking-wider mb-2 flex items-center gap-1.5">
              <HelpCircle class="h-4 w-4" />
              Lưu ý định dạng
            </span>
            <ul class="flex flex-col gap-2 list-disc pl-4 text-slate-600 font-medium">
        ...
        <div class="lg:col-span-7 flex flex-col gap-6">
          ...
          {/* ERROR BOARD */}
          {generationError && (
            <div class="rounded-xl bg-red-50 border border-red-200 p-4 flex gap-3 text-red-700 text-sm shadow-sm font-medium">
          ...
          {/* STEP PROGRESS BOARD */}
          {isGenerating && (
            <div class="rounded-xl glass-panel p-6 border border-slate-200/80 shadow-lg flex flex-col gap-6">
              <div>
                <h3 class="font-display font-bold text-lg text-slate-800 flex items-center gap-2">
                  <Loader2 class="h-5 w-5 animate-spin text-teal-600" />
                  Đang tiến hành tạo tài liệu
                </h3>
                <p class="text-xs text-slate-600 mt-1">Hệ thống đang chạy tuần tự các tác vụ tạo nội dung chi tiết.</p>
              </div>

              {/* Progress bar */}
              <div class="w-full bg-slate-200 h-2 rounded-full overflow-hidden border border-slate-300">
                <div 
                  class="bg-gradient-to-r from-teal-600 to-emerald-500 h-full rounded-full transition-all duration-500"
                  style={{ width: `${((currentStepIndex + 1) / steps.length) * 100}%` }}
                ></div>
              </div>

              {/* Substep status text */}
              <div class="flex items-center gap-2.5 p-3 rounded-lg bg-teal-50 border border-teal-200 text-xs text-teal-800 font-semibold">
                ...
                {steps.map((step, idx) => (
                  ...
                  >
                    ...
                    <div class="text-xs font-medium uppercase tracking-wider">
                      {step.status === 'running' && <span class="text-teal-600 animate-pulse font-bold">đang chạy...</span>}
                      {step.status === 'done' && <span class="text-emerald-600 font-bold">xong</span>}
                      {step.status === 'error' && <span class="text-red-600 font-bold">lỗi</span>}
                      {step.status === 'idle' && <span class="text-slate-400 font-semibold">chờ</span>}
                    </div>
                  </div>
                ))}
              </div>
          ...
          {!isGenerating && generatedDocBlob && (
            <div class="rounded-xl glass-panel p-6 border border-emerald-200 shadow-xl shadow-emerald-500/5 flex flex-col items-center text-center gap-5">
              ...
              <div>
                <h3 class="font-display font-extrabold text-xl text-slate-800">
                  Biên soạn tài liệu ôn tập thành công!
                </h3>
                <p class="text-xs text-slate-600 mt-1.5 max-w-md mx-auto leading-relaxed font-medium">
              ...
              <div class="flex flex-col sm:flex-row gap-3 w-full max-w-sm mt-2">
                <button
                  ...
                  class="flex-1 py-3 px-4 rounded-xl bg-white hover:bg-slate-50 border border-slate-200 text-slate-700 font-extrabold text-sm tracking-wider flex items-center justify-center gap-2 transition-all shadow-sm"
                >
                  <Eye class="h-4 w-4" />
                ...
                <button
                  ...
                  class="flex-1 py-3 px-4 rounded-xl bg-gradient-to-r from-teal-600 to-emerald-500 hover:from-teal-500 hover:to-emerald-400 text-white font-extrabold text-sm tracking-wider uppercase flex items-center justify-center gap-2 shadow-lg shadow-teal-600/10 transition-all border border-teal-500/20 focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2 focus:ring-offset-slate-50"
                >
          ...
          {!isGenerating && !generatedDocBlob && (
            <div class="rounded-xl glass-panel p-12 border border-slate-200 text-center flex flex-col items-center justify-center gap-4 min-h-[360px] shadow-inner relative overflow-hidden bg-white">
              <div class="absolute -right-32 -bottom-32 w-64 h-64 bg-teal-50/5 rounded-full blur-3xl"></div>
              <div class="h-16 w-16 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-600 border border-slate-200 shadow-inner">
                <Beaker class="h-8 w-8 stroke-[1.5]" />
              </div>
              <div>
                <h3 class="font-display font-bold text-lg text-slate-800">Sẵn sàng biên soạn tài liệu</h3>
                <p class="text-xs text-slate-600 max-w-sm mt-1 mx-auto leading-relaxed font-semibold">
          ...
          {accumulatedMarkdown && (showPreview || isGenerating) && (
            <div class="rounded-xl glass-panel border border-slate-200/80 shadow-lg overflow-hidden flex flex-col h-[500px] bg-white">
              {/* Header */}
              <div class="px-5 py-3 border-b border-slate-200 bg-slate-50 flex items-center justify-between">
                <span class="text-xs font-extrabold text-slate-600 uppercase tracking-wider flex items-center gap-1.5">
                  <FileText class="h-4 w-4 text-teal-600" />
                  Bản xem thử Markdown ({isGenerating ? 'Đang tạo...' : 'Hoàn thành'})
                </span>
                
                {isGenerating && (
                  <span class="h-2 w-2 rounded-full bg-teal-600 animate-ping"></span>
                )}
              </div>
              
              {/* Content stream view */}
              <div class="p-6 overflow-y-auto flex-1 font-mono text-xs leading-relaxed text-slate-800 bg-slate-50/70 select-text">
